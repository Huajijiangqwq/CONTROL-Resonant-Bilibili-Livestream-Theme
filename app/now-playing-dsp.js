(function (root, factory) {
  ...
  else root.NowPlayingDSP = factory(null);
  ...
  class SpectrumAnalyzer {
    constructor(rate = 48000, channels = 2) {
      ...
      // Fine output shares the exact same FFT, stereo power, window and hop.
      this.fineFilters = Array.from({ length: 240 }, (_, i) => {
        const lo = (MIN_HZ * Math.pow(MAX_HZ / MIN_HZ, i / 240) * this.size) / rate,
          hi = (MIN_HZ * Math.pow(MAX_HZ / MIN_HZ, (i + 1) / 240) * this.size) / rate,
          weights = [];
        for (
          let bin = Math.max(1, Math.floor(lo - 0.5));
          bin <= Math.min(this.size / 2 - 1, Math.ceil(hi + 0.5));
    ...
    measure() {
      ...
      const fineDb = this.fineFilters.map((weights) => {
        let sum = 0,
          weight = 0;
        for (const [bin, w] of weights) {
          sum += this.power[bin] * w;
